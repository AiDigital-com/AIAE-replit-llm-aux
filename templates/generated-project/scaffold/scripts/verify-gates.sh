...
if [ -f agent-payload.skills ]; then
  [ -x "${SCRIPT_DIR}/lib/check-agent-surfaces.sh" ] \
    || fail "Missing executable scripts/lib/check-agent-surfaces.sh"
  agent_surface_mode="$(bash "${SCRIPT_DIR}/lib/check-agent-surfaces.sh" .)"
  expected_payload="$(grep -vE '^[[:space:]]*(#|$)' agent-payload.skills | LC_ALL=C sort)"
  registries=(.claude/skills)
  [ "$agent_surface_mode" != "active" ] || registries+=(.agents/skills)
  for registry in "${registries[@]}"; do
    [ -d "$registry" ] || fail "Missing runtime skill registry: $registry"
    actual_payload="$(
      find "$registry" -mindepth 2 -maxdepth 2 -name SKILL.md -print \
        | sed "s#^${registry}/##; s#/SKILL.md\$##" \
        | LC_ALL=C sort
    )"
    [ "$actual_payload" = "$expected_payload" ] \
      || fail "$registry does not exactly match agent-payload.skills"
  done
fi
...
if [ -d backend/application/src/main/resources ]; then
  ...
  if grep -RInE 'app_user|app_user_role|user_roles' backend/migrations/src/main/resources 2>/dev/null | grep -q .; then
...
if [ -d backend ] && [ "${SCAFFOLD_SOURCE}" -eq 0 ]; then
  ...
  if [ -d backend/migrations/src/main/resources/db/changelog ]; then
