...
TEMPLATE_SOURCE_ROOT="${TEMPLATE_REPO_ROOT:-${ROOT}}"
...
if [ -n "${SCAFFOLD}" ] && [ -d "${SCAFFOLD}/scripts" ]; then
  ...
  RUNTIME_SCRIPTS=(
    ...
    apply-package-name.sh strip-scaffold-samples.sh remove-cache-management.sh
    remove-usage-logging.sh materialize-project.sh
...
# Untrack template-only control-plane paths from git index (if tracked).
# AGENTS.md, replit.md, and .agents/ are the committed active Replit runtime;
# engineering handoff removes them later through its explicit cleanup command.
cd "${ROOT}"
if [ -d .git ]; then
  for path in templates custom_instruction main.py pyproject.toml uv.lock; do
...
SCAFFOLD_ROOT="${SCAFFOLD}" \
  TEMPLATE_REPO_ROOT="${TEMPLATE_SOURCE_ROOT}" \
